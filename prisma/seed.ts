...
const dataBaruDibuat: Array<{ nama: string; nik: string; username: string }> = [];

// --- HELPER FUNCTIONS ---

const parseNamaOrtu = (namaOrtu: string) => {
 ...
 */
async function getOrCreateAyahAccount(namaAyah: string, posyanduId?: number) {
  if (!namaAyah || namaAyah === '-' || namaAyah.trim() === '') return null;
 ...
 */
async function getOrCreateIbuAccount(namaIbu: string, posyanduId?: number) {
  if (!namaIbu || namaIbu === '-' || namaIbu.trim() === '') return null;
 ...
 */
async function getOrCreateOrtuProfile(namaIbu: string, namaAyah: string, posyanduId?: number) {
  if ((!namaIbu || namaIbu === '-') && (!namaAyah || namaAyah === '-')) return null;
  ...
  try {
    ...
    if (userAyah) whereClause.userAyahId = userAyah.id;
    if (userIbu) whereClause.userIbuId = userIbu.id;
...
async function seedFileUtama() {
  ...
  const results: any[] = [];
  return new Promise<{ berhasil: number }>((resolve) => {
...
async function main() {
  ...
  const staffAccounts = [
    { username: 'superadmin', role: 'SUPER_ADMIN' as const, namaLengkap: 'Super Admin', posyanduId: null },
    { username: 'admin_desa', role: 'ADMIN' as const, namaLengkap: 'Admin Desa', posyanduId: defaultPosyandu.id },
    { username: 'bidan_desa', role: 'TENAGA_KESEHATAN' as const, namaLengkap: 'Bidan Desa', posyanduId: null },
    { username: 'kader_posyandu', role: 'KADER_POSYANDU' as const, namaLengkap: 'Kader Posyandu', posyanduId: defaultPosyandu.id },
  ];
