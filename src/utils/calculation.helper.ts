import { WHO_STANDARDS, type LMSData } from './who-standards.js';

/**
 * Calculate Age in Months
 */
export function calculateAgeInMonths(birthDate: Date, measurementDate: Date): number {
...
function interpolateLMS(target: number, data: LMSData[], key: 'age' | 'length'): LMSData | null {
  ...
  if (!lower) return upper!; // Targets before range
  if (!upper) return lower!; // Targets after range
 ...
 */
export function determineStatus(zScore: number, type: 'BB/U' | 'TB/U' | 'BB/TB' | 'LK/U'): string {
  ...
  if (type === 'BB/TB') {
    ...
    if (zScore > 1) return "Risiko Gizi Lebih";
    return "Gizi Baik";
  }
  ...
  if (type === 'LK/U') {
    ...
    if (zScore < -3) return "Mikrosefalus Berat";
    if (zScore < -2) return "Mikrosefalus";
    if (zScore > 3) return "Makrosefalus Berat";
    if (zScore > 2) return "Makrosefalus";
    return "Normal";
  }
