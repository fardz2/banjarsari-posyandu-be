...
export const getAllPosyanduService = async (
  requestingUser: UserContext
): Promise<PosyanduResponse[]> => {
  ...
  const posyanduWithKaderCount = await Promise.all(
    posyandu.map(async (p) => {
...
export const getPosyanduByIdService = async (
  id: number,
  requestingUser: UserContext
): Promise<PosyanduResponse> => {
...
export const createPosyanduService = async (
  data: CreatePosyanduInput,
  requestingUser: UserContext
): Promise<PosyanduResponse> => {
  // Only super admin can create posyandu
  requirePermission(
    canManagePosyandu(requestingUser.role),
    'Hanya Super Admin yang bisa membuat posyandu baru'
  );
...
export const updatePosyanduService = async (
  id: number,
  ...
  requirePermission(
    canManagePosyandu(requestingUser.role),
    'Hanya Super Admin yang bisa update posyandu'
  );
...
export const deletePosyanduService = async (
  id: number,
  requestingUser: UserContext
): Promise<void> => {
  // Get existing posyandu
  ...
  requirePermission(
    canManagePosyandu(requestingUser.role),
    'Hanya Super Admin yang bisa delete posyandu'
  );
  ...
  if (posyandu) {
    const totalData =
      posyandu._count.users +
      posyandu._count.anak +
      posyandu._count.ibuHamil;
    if (totalData > 0) {
      throw new Error(
        `Tidak bisa delete posyandu yang masih memiliki data (${posyandu._count.users} users, ${posyandu._count.anak} anak, ${posyandu._count.ibuHamil} ibu hamil)`
      );
    }
  }
