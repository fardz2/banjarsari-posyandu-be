...
export const getOrtuById = async (c: Context) => {
  try {
...
export const getMyOrtuProfile = async (c: Context) => {
  try {
    const user = c.get('user') as UserContext;
...
export const updateOrtu = async (c: Context) => {
  try {
    const id = parseInt(c.req.param('id'));
    const user = c.get('user') as UserContext;
    const body = await c.req.json<UpdateOrtuInput>();
...
export const updateMyOrtuProfile = async (c: Context) => {
  try {
    const user = c.get('user') as UserContext;
    const body = await c.req.json<UpdateOrtuInput>();
...
export const deleteOrtu = async (c: Context) => {
  try {
    const id = parseInt(c.req.param('id'));
    const user = c.get('user') as UserContext;
